...
class CL(nn.Module):
    ...
    def get_negative_mask(self, batch_size, labels1=None, labels2=None):
        if labels2 is None:
            ...
            mask = torch.eq(labels1, labels1.T).float().cuda()#生成对角阵
        else:
            labels1 = labels1.contiguous().view(-1, 1)
    ...
    def pos_loss(self, self_predictions, pos_predictions,neg, labels1=None, labels2=None):
        nb = 1
        ...
        # mask = torch.rand(similarity_m.shape[0],similarity_m.shape[1]).round().cuda()
        ...
        # neg = neg.reshape(pos_predictions.shape[0],nb,pos_predictions.shape[1])
     
        similarity_n = F.cosine_similarity(self_predictions, neg, dim=-1)
        


        neg = similarity_n.sum()/self.temperature
        print(neg.shape)


        pos_loss = (- pos +  neg) / mask.sum(-1) 
        pos_loss = pos_loss.mean()
        return pos_loss

    def forward(self, x1, x2,x3 ,labels1=None, labels2=None):
        # x1 = self.projection(x1)
        # x2 = self.projection(x2)
        loss = self.pos_loss(x1, x2, x3,labels1, labels2)
        return loss
